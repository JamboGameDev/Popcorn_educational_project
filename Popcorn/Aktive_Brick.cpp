#include "Aktive_Brick.h"


HPEN AAktive_Brick::Fading_Red_Brick_Pens[Max_Fade_Step]; 
HBRUSH AAktive_Brick::Fading_Red_Brick_Brushes[Max_Fade_Step]; 
HPEN AAktive_Brick::Fading_Blue_Brick_Pens[Max_Fade_Step]; 
HBRUSH AAktive_Brick::Fading_Blue_Brick_Brushes[Max_Fade_Step]; 


//AAktive_Brick 
AAktive_Brick::AAktive_Brick(EBrick_Type brick_type)
: Fade_Step(0), Brick_Type(brick_type) 
{
}
//------------------------------------------------------------------------------------------------------------
void AAktive_Brick::Act() 
{
...
//------------------------------------------------------------------------------------------------------------
void AAktive_Brick::Draw(HDC hdc, RECT &paint_area) 
{
...
//------------------------------------------------------------------------------------------------------------
void AAktive_Brick::Setup_Colors()    
{
	int i;
	int Max_Step = Max_Fade_Step;
	HPEN pen;
	HBRUSH brush;
...
//------------------------------------------------------------------------------------------------------------
unsigned char AAktive_Brick::Get_Fading_Channel(unsigned char color, unsigned char bg_color, int step)
{// функция вычисление угосающего кирпича(одного из компонента цвета) 
	return color - step * (color - bg_color) / (Max_Fade_Step - 1); 
}
//------------------------------------------------------------------------------------------------------------
void AAktive_Brick::Get_Fading_Color(const AColor &color, int step, HPEN &pen, HBRUSH &brush)  
{
	unsigned char r, g, b;
